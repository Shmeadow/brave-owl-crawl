...
import { MOBILE_HORIZONTAL_SIDEBAR_HEIGHT } from "@/lib/constants"; // Removed MOBILE_HEADER_EFFECTIVE_HEIGHT, MOBILE_HEADER_SIDEBAR_GAP
...
export function Sidebar({ isMobile }: SidebarProps) {
  ...
  return (
    <div
      ref={sidebarRef}
      className={cn(
        "flex", // Removed fixed, bg-card/60, backdrop-blur-xl, border, shadow-xl, transition-all, duration-300, ease-in-out
        isMobile ?
          `left-0 right-0 flex-row justify-around p-0.5 gap-0.5 rounded-none overflow-x-auto` : // Mobile: horizontal, below header with new gap
