...
export function useNotes() {
  ...
  useEffect(() => {
    if (authLoading) {
      console.log("[useNotes] Auth loading, skipping note load.");
      return;
    }

    const loadNotes = async () => {
      setLoading(true);
      console.log(`[useNotes] Loading notes for room ID: ${currentRoomId || 'personal space'}`);
      if (session && supabase) {
        setIsLoggedInMode(true);
        
        let fetchedNotes: NoteData[] = [];
        if (currentRoomId) {
          console.log(`[useNotes] Fetching notes for room: ${currentRoomId}`);
          ...
          if (fetchError) {
            ...
            console.error("[useNotes] Error fetching notes (Supabase, room):", fetchError);
          } else {
            fetchedNotes = roomNotes as NoteData[];
            console.log(`[useNotes] Fetched ${fetchedNotes.length} notes for room ${currentRoomId}.`);
          }
        } else {
          console.log("[useNotes] Fetching personal notes (room_id is NULL).");
          ...
          if (fetchError) {
            ...
            console.error("[useNotes] Error fetching notes (Supabase, personal):", fetchError);
          } else {
            fetchedNotes = personalNotes as NoteData[];
            console.log(`[useNotes] Fetched ${fetchedNotes.length} personal notes.`);
          }
          ...
          try {
            ...
            console.error("[useNotes] Error parsing local storage notes:", e);
            localNotes = [];
          }

          if (localNotes.length > 0) {
            console.log(`[useNotes] Found ${localNotes.length} local notes. Attempting migration...`);
            ...
            if (toInsert.length > 0) {
              console.log(`[useNotes] Migrating ${toInsert.length} unique local notes.`);
              ...
              if (insertError) {
                console.error("[useNotes] Error migrating local notes to Supabase:", insertError);
                ...
                console.log(`[useNotes] Successfully migrated ${newSupabaseNotes.length} local notes.`);
              }
            } else {
              localStorage.removeItem(LOCAL_STORAGE_KEY); // Clear if all already exist
              console.log("[useNotes] No unique local notes to migrate, clearing local storage.");
            }
          }
        }
        ...
        console.log(`[useNotes] Loaded ${loadedNotes.length} notes from local storage (guest mode).`);
        if (loadedNotes.length === 0) {
          toast.info("You are browsing notes as a guest. Your notes will be saved locally.");
        }
      }
      setLoading(false);
      console.log("[useNotes] Note loading complete.");
    };
  ...
  useEffect(() => {
    if (!isLoggedInMode && !loading) {
      ...
      console.log(`[useNotes] Saved ${notes.length} notes to local storage (guest mode).`);
    }
  ...
  const handleAddNote = useCallback(async (content: string) => {
    ...
    if (isLoggedInMode && supabase) {
      ...
      if (error) {
        ...
        console.error("[useNotes] Error adding note (Supabase):", error);
      } else if (data) {
  ...
  const handleDeleteNote = useCallback(async (noteId: string) => {
    if (isLoggedInMode && session && supabase) {
      ...
      if (error) {
        ...
        console.error("[useNotes] Error deleting note (Supabase):", error);
      } else {
  ...
  const handleToggleStar = useCallback(async (noteId: string) => {
    ...
    if (isLoggedInMode && session && supabase) {
      ...
      if (error) {
        ...
        console.error("[useNotes] Error updating star status (Supabase):", error);
      } else if (data) {
